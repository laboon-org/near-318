...
const Introduction = observer(() => {
  ...
  const [toggleTicketListModal, setToggleTicketListModal] = useState<boolean>(false);
  ...
  useEffect(() => {
    let isMounted = true;
    if (isMounted) {
      if (walletStore.isSignedIn && roundStore.rounds.length > 0) {
        loadTicketList();
      }
    }
    ...
    <article className={`${styles['intro-wrap']}`}>
      {toggleTicketListModal && playerList.length > 0 &&
        <ModalWrap setToggleModal={setToggleTicketListModal}>
          <BoughtTicketListM setToggleModal={setToggleTicketListModal} playerList={playerList[0]}/>
        </ModalWrap>
      }
          ...
          <div className={`${styles['upper-part']}`}>
            <div className={`${styles['content']}`}>
              ...
              <div className={`${styles['info']}`}>
                <div>
                  <p>Round:</p>
                  <span>#{roundStore.rounds.length > 0 ? roundStore.rounds[roundStore.rounds.length - 1].round : ''}</span>
                </div>
                <div className={`${styles['seperation']}`}></div>
                <div>
                  <p>Draw: </p>
                  <span>{roundStore.rounds.length > 0 ? formatDateShort(roundStore.rounds[roundStore.rounds.length - 1].endTime) : ''}</span>
                </div>
              </div>
            </div>
          </div>
          <div className={`${styles['lower-part']}`}>
            {playerList.length > 0 && playerList[0].players && playerList[0].players.length > 0 && (
              <div className={`${styles['ticket-info']}`}>
                <p>
                  You have 
                  <span>{` ${playerList[0].players[0].tickets.length} `}</span>
                  {playerList[0].players[0].tickets.length < 2 ? 'ticket' : 'tickets'} this round
                </p>
                <button 
                  type='button'
