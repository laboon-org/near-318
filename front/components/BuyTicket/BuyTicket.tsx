...
const BuyTicket = observer(() => {
  ...
  const handleBuyTicket = async (chosenPrice: number | undefined) => {
    try {
      if (
        ...
        chosenPrice <= Number(walletStore.accountBalance)
      ) {
        ...
        if (contractAddress && walletStore) {
          ...
          const result = await contract.addTickets({
            chosenNumber: chosenNumbers,
            betAmount: chosenPrice,
          });
          console.log(result);
  ...
  }, [router]); // add dependencies here

  useEffect(() => {
    handleTransaction();
  }, [handleTransaction]); // add handleTransaction as dependency
  ...
  }, [walletStore, roundStore]); // add dependencies here

  useEffect(() => {
    loadTicketList();
  }, [loadTicketList]); // add loadTicketList as dependency
  ...
  useEffect(() => {
    let isMounted = true;
    if (isMounted && roundStore.rounds.length > 0 && walletStore.isSignedIn) {
      loadTicketList();
    }
    return () => {
      isMounted = false;
            ...
            <div
              className={`${styles["choices"]} ${
                (!walletStore.isSignedIn ||
                  !conditionStore.availableToBuyTicket) &&
                "disabled"
              }`}
            >
                  ...
                  {bingoNumbers.map((bingoNum) => (
                    <button
                      ...
                      className={`
                        ${styles["number"]}
                        ${
                          (checkSelectedNumber([bingoNum]) ||
                            checkBoughtNumber([bingoNum])) &&
                  ...
                  {bingoNumbers.map((bingoNum) => (
                    <button
                      ...
                      className={`
                        ${styles["number"]}
                        ${
                          (checkSelectedNumber([bingoNum, bingoNum]) ||
                            checkBoughtNumber([bingoNum, bingoNum])) &&
                          "check-mark-container"
                        }
                      `}
              ...
              <div className={`${styles["choice-numbers-wrap"]}`}>
                ...
                <div
                  className={`${styles["choice-numbers"]} ${styles["triple"]}`}
                >
                  {bingoNumbers.map((bingoNum) => (
                    <button
                      type="button"
                      ...
                      ${
                        (checkSelectedNumber([bingoNum, bingoNum, bingoNum]) ||
                          checkBoughtNumber([bingoNum, bingoNum, bingoNum])) &&
                        "check-mark-container"
                      }`}
                      onClick={() =>
                        setBingoNumbers([bingoNum, bingoNum, bingoNum])
                      }
                    >
                      {checkBoughtNumber([bingoNum, bingoNum, bingoNum]) && (
                        <BoughtMark />
                  ...
                  <button
                    ...
                    className={`
                      ...
                      ${styles["optional"]}
                      ${
                        (checkSelectedNumber([0, 0, 0]) ||
                          checkBoughtNumber([0, 0, 0])) &&
                        "check-mark-container"
                      }`}
